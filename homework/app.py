...
def samples(sim):
    ...
    # val1=filter(lambda a: a != 0, val)
    ind=sorted(range(len(val)), key=lambda k: val[k])

    # val1={i:df.values for i in df.values if i!=0}
    # new_df={"out_id":df["out_id"],"sample_value":df[sim]}

    # return jsonify(list(val))
    # return jsonify(out_id=new_df["out_id"].values,sample_value=new_df["sample_value"].values)
    # return jsonify(list(oid),list(val))
